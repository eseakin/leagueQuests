...
class QuestIcon extends Component {
  ...
  render() {
  const { handleClick, onMouseEnter, onMouseLeave, quest, activeQuest = {}, i, questLineIndex } = this.props;
  ...
  const style = {
    ...
    return (
      <div style={style} className='quest' onMouseEnter={() => onMouseEnter(i)} onMouseLeave={onMouseLeave} onClick={() => handleClick(questLineIndex, i)}>
        <Card style={{boxShadow: outline, background: '#222'}}>
          <div style={{width: 70, height: 70, background: 'black'}}>
          {quest.completion > 0 && <Image src={quest.iconPath} fluid centered />}
          </div>
          <span>
