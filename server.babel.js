...
//   region: 'NA1',
//   summonerId: '29549543',
//   quests: new Array(23).fill({completion: 0, champ: '', time: ''})
// }
// db.ref('/users/NA1/ESCAPE GOAT').set(escapeGoat)
...
const checkPriorQuestCompletion = (result) => {
  const priorResults = result.user.quests[result.user.currentQuestId].completion;
  ...
  if(priorResults < completion) {
    ...
    const update = {};
    update[`quests/${result.questId}/completion`] = completion;
    update[`quests/${result.questId}/champ`] = result.champ;
    update[`quests/${result.questId}/time`] = result.time;
